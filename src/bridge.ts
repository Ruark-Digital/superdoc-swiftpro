 ...
 * @param hostOrigin  the expected origin (VITE_HOST_ORIGIN); anything else is dropped
 */
export function parseHostMessage(event: MessageEvent, hostOrigin: string): SuperdocInit | null {
  // Security: only ever trust messages from the configured host origin.
